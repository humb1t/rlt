...
pub enum BaselineError {
    ...
    #[error("baseline is not comparable: rate limit mismatch (current={}, baseline={})",
        .current.map_or("unlimited".into(), |v| format!("{v}/s")),
        .baseline.map_or("unlimited".into(), |v| format!("{v}/s")))]
...
pub enum CollectorError {
    #[error("latency value overflows histogram input: {}", humantime::format_duration(*.latency))]
    LatencyTooLarge { latency: Duration },

    #[error("failed to record latency into histogram")]
    HistogramRecord(#[source] hdrhistogram::RecordError),
}
